...
def draw_box_label(img, bbox_cv2, box_color=(0, 255, 255), obj_class=1, scores=0, show_label=True):
    '''
    ...
    if show_label:
        ...
        name += " score : "+str(scores)
        # Draw a filled box on top of the bounding box (as the background for the labels)
        (w, h), _ = cv2.getTextSize(name, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 1)
       # cv2.rectangle(img, (left-3, top-50,), (left+w, top), box_color, 1)
        cv2.putText(img, name, (left, top-2),
                    cv2.FONT_HERSHEY_COMPLEX, 0.6, box_color, 1)

        #cv2.rectangle(img, (left-2, top-45), (right+2, top), box_color, -1, 1)
        # Output the labels that show the x and y coordinates of the bounding box center.
        # text_x = 'x='+str((left+right)/2)
        # cv2.putText(img, text_x, (left, top-25), font,
        #             font_size, font_color, 1, cv2.LINE_AA)
        # # cv2.putText(img, name, (left-10, top-40), font,
        # #             font_size, font_color, 1, cv2.LINE_AA)
        # text_y = 'y='+str((top+bottom)/2)
        # cv2.putText(img, text_y, (left, top-5), font,
        #             font_size, font_color, 1, cv2.LINE_AA)
    return img
...
def draw_box_label_on_image(img, obj_boxes_list, box_color=(0, 255, 255), show_label=True):
    ...
    for bbox_cv2 in obj_boxes_list:
        ...
        if show_label:
            # Draw a filled box on top of the bounding box (as the background for the labels)
            cv2.rectangle(img, (left-2, top-45),
                          (right+2, top), box_color, -1, 1)
            # Output the labels that show the x and y coordinates of the bounding box center.
            text_x = 'x='+str((left+right)/2)
            cv2.putText(img, text_x, (left, top-25), font,
                        font_size, font_color, 1, cv2.LINE_AA)
